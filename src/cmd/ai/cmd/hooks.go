...
Per SPEC.md §3.10 + §10.2.`,
	...
	install.Flags().BoolVar(&installCopilot, "copilot", false, "create ~/.copilot/instructions/constitution.md symlink → ~/.ai/Constitution.runtime.md")

	// Wrap RunE to inject --copilot dispatch before the original handler runs.
	origRunE := install.RunE
	install.RunE = func(cmd *cobra.Command, args []string) error {
		if installCopilot {
			...
			return runHooksCopilotInstall(aiRoot, home)
		}
...
// symlink pointing at <aiRoot>/Constitution.runtime.md.
//
// Behaviour:
//   - Returns an error if Constitution.runtime.md does not exist yet (guides
//     the user to run `ai generate runtime` first).
//   - Creates ~/.copilot/instructions/ if absent.
//   - If the symlink already exists and points to the correct target, returns
//     nil (idempotent).
//   - If the symlink is stale (wrong target or dangling), removes and recreates.
...
func runHooksCopilotInstall(aiRoot, home string) error {
	...
	if existing, err := os.Readlink(symlinkPath); err == nil {
		if existing == runtimeFile {
			...
			fmt.Printf("Copilot instruction symlink already up to date: %s\n", symlinkPath)
			return nil
		}
	...
	fmt.Printf("Copilot instruction symlink created: %s\n", symlinkPath)
	return nil
}
